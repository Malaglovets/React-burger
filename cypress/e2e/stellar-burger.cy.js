...
const BurgerConstructor_order = '[class^=BurgerConstructor_order]';
const cratorBun = 'Краторная булка N-200i'
...
//проверка перетаскивания ингредиетов
  it('should drag ang drop ingredients', function() {
    ...
    cy.contains(cratorBun ).should('be.not.visible')
    ...
    cy.contains(cratorBun ).should('be.visible')
  })
//полная проверка оформления заказа
  it('should drag ingredients, login, make order again, check preloader and popup with order ', function() {
    ...
    cy.get('a').contains(cratorBun).trigger('dragstart', { dataTransfer });
    cy.get(BurgerConstructor_order).trigger('drop', { dataTransfer });
